 ...
 */
export async function donationRoutes(
  ...
  fastify.put(
    ...
    async (request, reply) => {
      try {
        ...
        if (
          updates.type &&
          !Object.values(DonationType).includes(updates.type)
        ) {
          return reply.status(400).send({ error: "Invalid donation type" });
        }
        ...
        const updatedDonation = donationDataService.update(id, updates);
